...
// 1. Remove modal block inside activeTab.startsWith('master-')
const insideModalStart = "            {/* Stable Non-Flickering Modal UI */}";
const insideModalEnd = "            )}";

const idx1 = code.indexOf(insideModalStart);

if (idx1 === -1) {
  console.error("Could not find insideModalStart in App.jsx!");
  process.exit(1);
}

// Find the closing of this modal block before activeTab === 'manual-repayment'
const nextTabIdx = code.indexOf("{activeTab === 'manual-repayment'", idx1);
const idx2 = code.lastIndexOf(")}", nextTabIdx);

if (idx2 === -1) {
  console.error("Could not find idx2 for inside modal!");
  process.exit(1);
}

// Remove the modal from inside the card
code = code.substring(0, idx1) + code.substring(idx2 + 2);
console.log("Removed modal from inside card container!");

// 2. Add Top-Level Modal right before line 5263 (tracking modal end)
const trackingModalEnd = "        </div>\n      )}\n        </div>\n      </main>";

const modalMarkup = `        {/* Top-Level Stable Non-Flickering Master Modal UI */}
              ...
              >
                ...
                <div style={{ background: 'var(--header-bg, #0B2545)', color: '#ffffff', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <h3 style={{ margin: 0, color: '#ffffff', fontSize: '1.1rem', fontWeight: 700 }}>
                    {isEditMode ? 'Edit' : 'Tambah'} {getMasterTitle(masterTab)}
                  </h3>
                  <button 
                    type="button"
...
const targetIdx = code.indexOf(trackingModalEnd);

if (targetIdx === -1) {
  console.error("Could not find trackingModalEnd in App.jsx!");
  process.exit(1);
}

code = code.substring(0, targetIdx) + modalMarkup + code.substring(targetIdx);
fs.writeFileSync('src/App.jsx', code, 'utf8');
console.log("Successfully moved Master Modal to Top-Level of App.jsx!");
